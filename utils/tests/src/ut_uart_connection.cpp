...
TEST_F(UartConnectionTest, constructor_with_empty_device_path_throws) {
	...
	EXPECT_THROW(
		UartConnection("", 115200, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_8, callback),
...
TEST_F(UartConnectionTest, constructor_with_null_callback_throws) {
	EXPECT_THROW(
		UartConnection("/dev/ttyUSB0", 115200, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_8, nullptr),
		std::invalid_argument
	);
}
...
TEST_F(UartConnectionTest, constructor_with_zero_baudrate_throws) {
	auto callback = [](uint8_t) {};
	EXPECT_THROW(
		UartConnection("/dev/ttyUSB0", 0, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_8, callback),
		std::invalid_argument
	);
}
...
TEST_F(UartConnectionTest, constructor_with_valid_parameters_succeeds) {
	...
	EXPECT_NO_THROW(
		UartConnection("/dev/ttyUSB0", 115200, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_8, callback)
	);
}

TEST_F(UartConnectionTest, is_open_returns_false_before_open) {
	auto callback = [&](uint8_t c) { callback_counter.increment(c); };
	UartConnection uart("/dev/ttyUSB0", 115200, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_8, callback);
	EXPECT_FALSE(uart.is_open());
}

TEST_F(UartConnectionTest, open_nonexistent_device_throws) {
	auto callback = [&](uint8_t c) { callback_counter.increment(c); };
	UartConnection uart("/dev/nonexistent_device", 115200, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_8, callback);
...
TEST_F(UartConnectionTest, move_operations_are_deleted) {
	auto callback = [&](uint8_t c) { callback_counter.increment(c); };
	UartConnection uart("/dev/ttyUSB0", 115200, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_8, callback);
	
	// Verify move operations are deleted
	static_assert(!std::is_move_constructible_v<UartConnection>,
		"UartConnection should not be move constructible");
	static_assert(!std::is_move_assignable_v<UartConnection>,
		"UartConnection should not be move assignable");
}
...
TEST_F(UartConnectionTest, constructor_stores_all_parameters) {
	...
	EXPECT_NO_THROW(
		UartConnection uart1("/dev/ttyUSB0", 9600, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_5, callback);
	);
	EXPECT_NO_THROW(
		UartConnection uart2("/dev/ttyS0", 115200, SERIAL_PARITY_ODD, SERIAL_STOPBITS_2, SERIAL_DATABITS_8, callback);
	);
	EXPECT_NO_THROW(
		UartConnection uart3("/dev/ttyACM0", 57600, SERIAL_PARITY_EVEN, SERIAL_STOPBITS_1, SERIAL_DATABITS_7, callback);
	);
}
...
TEST_F(UartConnectionTest, valid_parity_values_accepted) {
	...
	// Parity ODD
	EXPECT_NO_THROW(
		UartConnection("/dev/ttyUSB0", 115200, SERIAL_PARITY_ODD, SERIAL_STOPBITS_1, SERIAL_DATABITS_8, callback);
	);
	
	// Parity EVEN
	EXPECT_NO_THROW(
		UartConnection("/dev/ttyUSB0", 115200, SERIAL_PARITY_EVEN, SERIAL_STOPBITS_1, SERIAL_DATABITS_8, callback);
	);
}

TEST_F(UartConnectionTest, valid_stop_bits_accepted) {
	auto callback = [&](uint8_t c) { callback_counter.increment(c); };
	...
	// Stop bits 2
	EXPECT_NO_THROW(
		UartConnection("/dev/ttyUSB0", 115200, SERIAL_PARITY_NONE, SERIAL_STOPBITS_2, SERIAL_DATABITS_8, callback);
	);
}
...
TEST_F(UartConnectionTest, valid_data_bits_accepted) {
	...
	EXPECT_NO_THROW(
		UartConnection("/dev/ttyUSB0", 115200, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_5, callback);
	);
	EXPECT_NO_THROW(
		UartConnection("/dev/ttyUSB0", 115200, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_6, callback);
	);
	EXPECT_NO_THROW(
		UartConnection("/dev/ttyUSB0", 115200, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_7, callback);
	);
	EXPECT_NO_THROW(
		UartConnection("/dev/ttyUSB0", 115200, SERIAL_PARITY_NONE, SERIAL_STOPBITS_1, SERIAL_DATABITS_8, callback);
	);
